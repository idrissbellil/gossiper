...
type UserQuery struct {
	...
	withAuthtokens *PasswordTokenQuery
	withJobs       *JobQuery
...
// QueryAuthtokens chains the current query on the "authtokens" edge.
func (uq *UserQuery) QueryAuthtokens() *PasswordTokenQuery {
...
// the "authtokens" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithAuthtokens(opts ...func(*PasswordTokenQuery)) *UserQuery {
	query := (&PasswordTokenClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAuthtokens; query != nil {
		if err := uq.loadAuthtokens(ctx, query, nodes,
			func(n *User) { n.Edges.Authtokens = []*PasswordToken{} },
			func(n *User, e *PasswordToken) { n.Edges.Authtokens = append(n.Edges.Authtokens, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadAuthtokens(ctx context.Context, query *PasswordTokenQuery, nodes []*User, init func(*User), assign func(*User, *PasswordToken)) error {
...
func (uq *UserQuery) loadJobs(ctx context.Context, query *JobQuery, nodes []*User, init func(*User), assign func(*User, *Job)) error {
	...
	for _, n := range neighbors {
		fk := n.job_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "job_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "job_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
